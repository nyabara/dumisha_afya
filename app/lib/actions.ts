...
const FormSchema = z.object({
  ...
  startDate: z.string({
    invalid_type_error: 'Please select job application start adate.',
  }),

  endDate: z.string({
    invalid_type_error: 'Please select job application end adate.',
  }),
...
export async function updateJob(id: string, formData: FormData) {
  const { position, station, period, status } = UpdateJob.parse({
  ...
  try{
    await sql`
    UPDATE vacancies
    SET position = ${position}, station_id = ${station}, period =${period}, status = ${status}}
    WHERE id = ${id}
  `;
  } catch(error){
    return {
